...
async def sleep_start_handler(message: Message, state: FSMContext):
    ...
    if sleep_start:
        await message.answer("Вы уже записали время начала сна. Сначала завершите текущий сон, а затем можете записать новое начало сна.", reply_markup=create_main_menu())
        return

    current_time = datetime.now().strftime("%H:%M")
    save_sleep_start(user_id, current_time)
    await message.answer(f"Время начала сна записано: {current_time}", reply_markup=create_main_menu())
...
async def sleep_end_handler(message: Message, state: FSMContext):
    ...
    if not sleep_start:
        await message.answer("Сначала запишите время начала сна перед записью конца сна.", reply_markup=create_main_menu())
        return

    current_time = datetime.now().strftime("%H:%M")
    save_sleep_end(user_id, current_time)
    await message.answer(f"Время конца сна записано: {current_time}", reply_markup=create_main_menu())
